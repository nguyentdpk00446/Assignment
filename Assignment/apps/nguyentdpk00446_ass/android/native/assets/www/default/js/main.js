...
function Checklogin(){
	WL.SimpleDialog.show("Tiêu đề", 'Đăng nhập không đúng',
								[
								 	{
								 		text: 'Thử lại', handler:function()
								 		{
								 			alert('Vui lòng thử lại');
								 		}
								 	},
								 	{
								 		text: 'Quên mật khẩu', handler:function()
								 		{
								 			alert('Tôi đã quên mật khẩu')
								 		}
								 	}
								 ]
		...
		if(soLuongKetQua > 0){
			...
			for(i = 0; i<items.length; i++){
				text +="<tr>"+
					   ...
					   "<td>"+items[i].nganh+"</td>"+
					   "<td>"+"<input type='button' value='Xóa' idsinhvien = '"+items[i].id+"' />"+"</td>"
					   "</tr>";
			}
...
function insertSv(){
	...
	var invocationData = {
			adapter: 'MySqlAdapter',
	        procedure : "Themsv",
	        parameters : [name, maso, gender, birth, nganh, pass, access]
		};
	WL.Client.invokeProcedure(invocationData,{
